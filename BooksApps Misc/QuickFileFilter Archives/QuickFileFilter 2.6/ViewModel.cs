...
namespace QuickFileFilter
{
    class ViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        private void ApplyFilter()
        {
            ...
            NumFiles = view.Count == files.Count ? $"{view.Count}" : $"{view.Count} / {files.Count}";
        }
        ...
        private readonly ObservableCollection<FileNameAndSize> files = new ObservableCollection<FileNameAndSize>();
        public ObservableCollection<FileNameAndSize> Files => files;
        ...
        // Folder sets
        public ObservableCollection<FolderSet> FolderSets => model.FolderSets;
        ...
        public bool RegExp
        {
            ...
            set
            {
                if (_RegExp != value)
                {
                    ...
                    NotifyPropertyChanged(nameof(IsWholeWordsEnabled));
                    ApplyFilter();
                }
            }
        }

        public bool IsRegExEnabled => !WholeWords;
        ...
        public bool WholeWords
        {
            ...
            set
            {
                if (_WholeWords != value)
                {
                    ...
                    NotifyPropertyChanged(nameof(IsRegExEnabled));
                    ApplyFilter();
                }
            }
        }

        public bool IsWholeWordsEnabled => !RegExp;
        ...
        public string Error => this["Folder"] + this["Filter"];
            ...
            get
            {
                switch (columnName)
                {
                    ...
                    case "Filter":
                        ...
                        {
                            string r = Filter.StartsWith("-") ? Filter.Substring(1) : filter;

                            bool isValid = false;
                            try
                            {
                                var re = new Regex(r);      // Just to raise an exception
                                isValid = true;
                            }
                            catch (Exception)
                            {
        ...
        private bool CanCopyFolderPath(object obj)
        {
            if (window.FilesListView.SelectedItems.Count > 1)
            {
                ...
                if (totalSize < 1024L)
                    ...
                    totalSizeString = ((totalSize + 1023L) / 1024L).ToString("N0") + " Ko";
                else
                    totalSizeString = ((totalSize / 1024L + 1023L) / 1024L).ToString("N0") + " Mo";
                SelectionSize = $"{window.FilesListView.SelectedItems.Count} fichiers sélectionnés, " + totalSizeString;
            }
            else
                SelectionSize = "";
