...
Public Sub Importer()
  ...
  s1 = "Copiez un tableau d'items dans le presse-papiers avant d'importer des données."
  
  ' On vérifie que le presse-papiers contient bien la grille voulue
  If Not Clipboard.GetFormat(vbCFText) Then
    MsgBox s1, vbExclamation, App.Title
    Exit Sub
  End If
  ...
  Dim tiInd As Variant
  Dim tsClé As Variant
  tsClé = Array("it_langue", "it_theme", "it_secteur", "it_rubrique", "it_sousrub", "it_titre", "*it_libelle")
  tiInd = ChercheColonnes(c, tsClé)
  
  If IsEmpty(tiInd) Then
    MsgBox s1, vbExclamation, App.Title
    Exit Sub
  End If
  
  frmImport.Show
  frmImport.Caption = "Import d'items"
  ...
  For i = LBound(l) + 1 To UBound(l)
    If l(i) <> "" Then
      ...
      For j = UBound(tiInd) To LBound(tiInd) Step -1
        ...
        Else
          Select Case tsClé(j)
            Case "it_langue":   s = cLangage
            Case "it_theme":    s = "K"
            Case "it_secteur":  s = "1"
            Case "it_rubrique": s = "1"
            Case "it_sousrub":  s = ""
            Case "it_titre":    s = ""
          End Select
        End If
...
Public Sub ImportCallBack()
  ...
  For Each x In frmImport.lvImport.ListItems
    ...
    If iSerial = 0 Then
      ...
      ActiveTraçabilité False
      iMode = meAdd
      For Each s In Array("it_langue", "it_theme", "it_secteur", "it_rubrique", "it_sousrub", "it_titre", "it_libelle")
        colChamps(s) = x.SubItems(h(s).Index - 1)
      Next
