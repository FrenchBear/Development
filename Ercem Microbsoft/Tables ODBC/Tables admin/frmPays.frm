VERSION 5.00
Begin VB.Form frmPays 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Gestion des codes pays"
   ClientHeight    =   1935
   ClientLeft      =   150
   ClientTop       =   150
   ClientWidth     =   8415
   ...
   End
End
Attribute VB_Name = "frmPays"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
' frmPays
' Feuille de gestion des pays
' 12/02/1998 PV
...
Private cPay As BDCurseur                 ' Curseur courant
...
Private Sub ValidateQuery()
  ...
  Dim cPayQuery As New BDCurseur
  ActiveFormulaire False
  SetMessage "Recherche en cours…"
  Dim iErr As Long, sErr As String, sSQL As String
  On Error Resume Next
  sSQL = "select * from pays where " & sWP & " order by pay_pays"
  cPayQuery.OpenCurseur sSQL
  iErr = Err
  ...
  If Not cPay Is Nothing Then cPay.CloseCurseur
  Set cPay = cPayQuery
  sFinalSQL = sSQL
  ...
  If Not (cPay.EOF And cPay.BOF) Then
    iMode = meNavigate
    SetMessage
  Else
    iMode = meEmpty
...
Private Sub ValidateAdd()
  ...
  Dim cPayAdd As New BDCurseur
  cPayAdd.OpenCurseur "select * from pays where pay_pays=0"
  cPayAdd.AddNew
  If Not bPrépareEditAdd(cPayAdd, colColonnes, colChamps) Then cPayAdd.CloseCurseur: Exit Sub
  cPayAdd.CloseCurseur
  
  
  ' Tout est Ok, on sélectionne l'enregistrement qu'on vient d'ajouter
  If Not cPay Is Nothing Then cPay.CloseCurseur
  Set cPay = New BDCurseur
  sFinalSQL = "select * from pays where pay_pays=" & sFormToSQL(colChamps("pay_pays"), BDTypeINTEGER)
  cPay.OpenCurseur sFinalSQL
  cPay.MoveFirst
  ...
  If iMode = meAdd Then
    ...
    If sMsgTrace <> "" Then
      JournalUnix sTable & " " & colChamps("pay_pays"), "créé", sMsgTrace
    End If
  End If
...
Private Sub ValidateEdit()
  ...
  Assert Not cPay Is Nothing

  If Not bOkNullType(colColonnes, colChamps) Then Exit Sub
  If Not bOkValeurs Then Exit Sub
  cPay.Edit
  If Not bPrépareEditAdd(cPay, colColonnes, colChamps) Then Exit Sub
...
' Tout est Ok: si on est en train de faire une modif, on trace ce qui a été changé
  If iMode = meChange Then
    ...
    Next
    If sMsgTrace <> "" Then
      JournalUnix sTable & " " & colChamps("pay_pays"), "changé", sMsgTrace
    End If
  End If
...
Public Sub ActionChange()
  ...
  ActiveChamp "pay_pays", False
  ActiveChamp "pay_idmodif", False
  ActiveChamp "timestamp", False
  
  colChamps("pay_libelle").SetFocus
  AutoSelect colChamps("pay_libelle"), True
  bdSél.TabStop = False
...
Public Sub ActionDelete()
  ...
  If sMsgTrace <> "" Then
    JournalUnix sTable & " " & colChamps("pay_pays"), "détruit", sMsgTrace
  End If
...
Private Sub AjusteMouvements()
  ...
  bdSél.ButtonEnabled("First") = cPay.AbsolutePosition <> 1
  bdSél.ButtonEnabled("Previous") = cPay.AbsolutePosition <> 1
  bdSél.ButtonEnabled("Next") = Not cPay.EOF
  bdSél.ButtonEnabled("Last") = Not cPay.EOF
End Sub
...
Public Sub ActionNext()
  ...
  cPay.MoveNext
  AjusteMouvements
  If cPay.EOF Then cPay.MoveLast
  SynchroniseFormulaire
End Sub
...
Public Sub ActionPrevious()
  ...
  cPay.MovePrevious
  AjusteMouvements
  If cPay.BOF Then cPay.MoveFirst
  SynchroniseFormulaire
End Sub
    ...
    If Shift = 0 Then
      If iMode = meAdd Or iMode = meChange Or iMode = meQuery Then
        ...
        If Not ActiveControl.Locked Then
          Set C = ActiveControl
          C = colMémoirePays(ActiveControl.DataField)
          C.Refresh
          SendKeys "{Tab}"
        End If
        KeyCode = 0
...
Private Sub Form_Unload(Cancel As Integer)
  ...
  ' On recharge préventivement les pays
  PaysARecharger
End Sub
...
Private Sub SynchroniseFormulaire()
  If cPay Is Nothing Then
    EffaceFormulaire
  ElseIf cPay.BOF And cPay.EOF Then
    EffaceFormulaire
  Else
    Dim C As Control
    On Error GoTo pb
    For Each C In colChamps
      If IsNull(cPay.Fields(C.DataField)) Then
        C.Text = ""
      Else
        C.Text = cPay.Fields(C.DataField)
      End If
      C.Refresh
    Next
    On Error GoTo 0
    bdSél.NumRec = cPay.AbsolutePosition
  End If
  Exit Sub
...
Public Sub Copie()
  ...
  BDCopieSélection cPay, colChamps, bdSél
...
Public Sub Imprime()
  BDEtat sFinalSQL, drPays, True
End Sub

Public Sub Aperçu()
  BDEtat sFinalSQL, drPays, False
End Sub
...
Public Sub Mémorise()
  EffaceCollection colMémoirePays
  
  Dim t As Control
  For Each t In colChamps
    colMémoirePays.Add (t), t.DataField
  Next
End Sub
...
Public Sub Historique()
  ...
  Dim sWP As String
  sWP = "jou_domaine='" & sTable & " " & colChamps("pay_pays") & "'"
