...
Begin VB.Form frmParamètre 
   ...
   ScaleWidth      =   505
   Tag             =   "Données"
   Begin VB.CommandButton btnDerniersNos 
      Caption         =   "Derniers n°s"
      Height          =   375
      Left            =   6000
      TabIndex        =   55
      TabStop         =   0   'False
...
'  5/09/2001 PV Derniers paramètres, retouches état
...
Public Sub ActionSupprime()   ' Marque l'enregistrement supprimé (état S)
  ...
  Dim sMsgTrace As String
  sMsgTrace = colChamps("ger_indice") & "-> Etat S"
...
Private Sub btnDerniersNos_Click()
  Dim iN1 As Integer
  iN1 = BDCurseurExpress("select max(ger_indice) from germe")
  Dim iN2 As Integer
  iN2 = BDCurseurExpress("select max(ger_indice) from germe where ger_indice between 1 and 899")
  MsgBox "Dernier paramètre bactério créé: " & iN2 & vbCrLf & "Dernier paramère chimie créé: " & iN1, vbInformation, App.Title
End Sub
