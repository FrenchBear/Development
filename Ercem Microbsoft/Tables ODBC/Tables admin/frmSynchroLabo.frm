VERSION 5.00
Begin VB.Form frmSynchroLabo 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Synchro - Diffusion des résultats Labo"
   ClientHeight    =   1980
   ClientLeft      =   150
   ClientTop       =   150
   ClientWidth     =   8415
   ...
   End
End
Attribute VB_Name = "frmSynchroLabo"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
' frmSynchroLabo
' Synchro - Diffusion des résultats Labo
' 11/12/2000 NC
...
Private cSL As BDCurseur                  ' Curseur courant
...
Private Const sTable = "synchrolabo"
...
Private Sub ValidateQuery()
  ...
  Dim cSLQuery As New BDCurseur
  ActiveFormulaire False
  SetMessage "Recherche en cours…"
  Dim iErr As Long, sErr As String, sSQL As String
  On Error Resume Next
  sSQL = "select * from synchrolabo where " & sWP & ""
  cSLQuery.OpenCurseur sSQL
  iErr = Err
  ...
  If Not cSL Is Nothing Then cSL.CloseCurseur
  Set cSL = cSLQuery
  sFinalSQL = sSQL
  ...
  If Not (cSL.EOF And cSL.BOF) Then
    iMode = meNavigate
    SetMessage
  Else
    iMode = meEmpty
...
Private Sub ValidateAdd()
  ...
  Dim cSLAdd As New BDCurseur
  cSLAdd.OpenCurseur "select * from synchrolabo where sl_systeme='@'"
  cSLAdd.AddNew
  If Not bPrépareEditAdd(cSLAdd, colColonnes, colChamps) Then cSLAdd.CloseCurseur: Exit Sub
  cSLAdd.CloseCurseur
  
  
  ' Tout est Ok, on sélectionne l'enregistrement qu'on vient d'ajouter
  If Not cSL Is Nothing Then cSL.CloseCurseur
  Set cSL = New BDCurseur
  sFinalSQL = "select * from synchrolabo where sl_systeme=" & sFormToSQL(colChamps("sl_systeme"), BDTypeCHAR)
  cSL.OpenCurseur sFinalSQL
  cSL.MoveFirst
  ...
  If iMode = meAdd Then
    ...
    If sMsgTrace <> "" Then
      JournalUnix sTable & " " & colChamps("sl_systeme"), "créé", sMsgTrace
    End If
  End If
...
Private Sub ValidateEdit()
  ...
  Assert Not cSL Is Nothing

  If Not bOkNullType(colColonnes, colChamps) Then Exit Sub
  If Not bOkValeurs Then Exit Sub
  cSL.Edit
  If Not bPrépareEditAdd(cSL, colColonnes, colChamps) Then Exit Sub
  ...
  Next
  If sMsgTrace <> "" Then
    JournalUnix sTable & " " & colChamps("sl_systeme"), "changé", sMsgTrace
  End If
  End If
  ActiveFormulaire False
  bdSél.ButtonEnabled("Query") = True
...
Public Sub ActionDelete()
  ...
  If sMsgTrace <> "" Then
    JournalUnix sTable & " " & colChamps("sl_systeme"), "détruit", sMsgTrace
  End If
...
Private Sub AjusteMouvements()
  ...
  bdSél.ButtonEnabled("First") = cSL.AbsolutePosition <> 1
  bdSél.ButtonEnabled("Previous") = cSL.AbsolutePosition <> 1
  bdSél.ButtonEnabled("Next") = Not cSL.EOF
  bdSél.ButtonEnabled("Last") = Not cSL.EOF
End Sub
...
Private Sub SynchroniseFormulaire()
  If cSL Is Nothing Then
    EffaceFormulaire
  ElseIf cSL.BOF And cSL.EOF Then
    EffaceFormulaire
  Else
    Dim C As Control
    On Error GoTo pb
    For Each C In colChamps
      If IsNull(cSL.Fields(C.DataField)) Then
        C.Text = ""
      Else
        C.Text = cSL.Fields(C.DataField)
      End If
      C.Refresh
    Next
    On Error GoTo 0
    bdSél.NumRec = cSL.AbsolutePosition
  End If
  Exit Sub
...
Public Sub Copie()
  ...
  BDCopieSélection cSL, colChamps, bdSél
...
'Public Sub Imprime()
'  BDEtat sFinalSQL, drsynchrolabo, True
'End Sub
'
'Public Sub Aperçu()
'  BDEtat sFinalSQL, drsynchrolabo, False
'End Sub
...
Public Sub Historique()
  ...
  Dim sWP As String
  sWP = "jou_domaine='" & sTable & " " & colChamps("sl_systeme") & "'"
