...
'  5/09/2000 PV Sélection hiérarchique; AfficheDirNat via where-part
' 13/02/2001 PV Traçabilité des opérations générales
'  9/04/2001 PV Mécanisme de mémorisation (F5)
...
Private Sub ValidateQuery(Optional ByVal sWP As String)
  ...
  For Each t In colChamps
    If Not t.Locked And t.Text <> "" Then
      ...
      sWP = sWP & "(" & sFinal & ")"
      If t.DataField = "dna_etat" Then bWPIntègreEtat = True
    End If
  Next
  ...
  If Not bWPIntègreEtat Then sWP = sWP & " and dna_etat is null"
...
Private Function bOkValeurs() As Boolean
  ...
  If iSer <> 0 And iSer <> colChamps("dna_numdna") Then
    MsgBox "ATTENTION - Cette direction nationale existe déjà sous le n° " & iSer, vbExclamation, App.Title
    Exit Function
  End If
...
Private Sub ValidateAdd()
  ...
  InitTraçabilité colChamps, "dna", True
  
  ' Seuls les champs actifs sont mis à jour
  ActiveTraçabilité True
  ActiveChamp "dna_numdna", True
  
  If Not bPrépareEditAdd(cDnaAdd, colColonnes, colChamps) Then
  ...
  Next
  If sMsgTrace <> "" Then
    JournalUnix sTable & " " & colChamps("dna_numdna"), "créé", sMsgTrace
  End If
...
Private Sub ValidateEdit()
  ...
  If bOpGénérale Then
    ...
    sSQL = sSQL & ",timestamp=current,dna_datemodif=current,dna_heuremodif=" & Hour(Now) * 100 + Minute(Now) & ",dna_opermodif=" & iNumOpérateur & " where " & sFinalWP
  ...
  Else  ' Modification classique
    ...
    ' Traçabilité ancien style
    InitTraçabilité colChamps, "dna", False
    ActiveTraçabilité True     ' Pour mettre à jour la base
    
    If Not bPrépareEditAdd(cDna, colColonnes, colChamps) Then
      ActiveTraçabilité False
      Exit Sub
    End If
    ...
    If iMode = meChange Then
      ...
      Next
      If sMsgTrace <> "" Then
        JournalUnix sTable & " " & colChamps("dna_numdna"), "changé", sMsgTrace
      End If
    End If
  End If
...
Public Sub ActionSupprime()   ' Marque l'enregistrement supprimé (état S)
  ...
  If iNb > 0 Then
    MsgBox "Impossible de basculer la direction nationale à l'état S, il reste " & iNb & " direction(s) régionale(s) rattachée(s) pas l'état S.", vbExclamation, App.Title
    Exit Sub
  End If
  ...
  InitTraçabilité colChamps, "dna", False
  ActiveTraçabilité True     ' Pour mettre à jour la base
  bPrépareEditAdd cDna, colColonnes, colChamps
  ActiveTraçabilité False
  ...
  sMsgTrace = colChamps("dna_numdna") & " -> Etat S"
    If cDna!dna_etat = "S" Then
       If sMsgTrace <> "" Then
          JournalUnix sTable & " " & colChamps("dna_numdna"), "supprimé", sMsgTrace
       End If
    End If
  SynchroniseFormulaire   ' Affiche le timestamp
End Sub
...
Public Sub Récupère()   ' Récupère un enregistrement supprimé (état S)
  ...
  Dim sMsgTrace As String
  sMsgTrace = colChamps("dna_numdna") & " -> Récup"
  If sMsgTrace <> "" Then
    JournalUnix sTable & " " & colChamps("dna_numdna"), "récupéré", sMsgTrace
  End If
...
Public Sub ActionDétruit()    ' Détruit l'enregistrement
  ...
  If iNb > 0 Then
    MsgBox "Impossible de basculer la direction nationale à l'état S, il reste " & iNb & " direction(s) régionale(s) rattachée(s).", vbExclamation, App.Title
    Exit Sub
  End If
  ...
  JournalUnix sTable & " " & colChamps("dna_numdna"), "détruit", sMsgTrace
...
Private Sub AjusteMouvements()
  ...
  bdSél.ButtonEnabled("First") = cDna.AbsolutePosition <> 1
  bdSél.ButtonEnabled("Previous") = cDna.AbsolutePosition <> 1
  bdSél.ButtonEnabled("Next") = Not cDna.EOF
  bdSél.ButtonEnabled("Last") = Not cDna.EOF
End Sub
    ...
    If Shift = 0 Then
      If iMode = meAdd Or iMode = meChange Or iMode = meQuery Then
        ...
        If Not ActiveControl.Locked Then
          Set c = ActiveControl
          c = colMémoireDirNat(ActiveControl.DataField)
          c.Refresh
          SendKeys "{Tab}"
        End If
        KeyCode = 0
    ...
    ElseIf Shift = 1 Then
      ...
      For Each c In colChamps
        If Not c.Locked Then      ' On ne remplit pas les champs verrouillés
          c = colMémoireDirNat(c.DataField)
        End If
      Next
    End If
  Else
...
Private Sub SynchroniseFormulaire()
  ...
  Else
    ...
    bdSél.NumRec = cDna.AbsolutePosition
...
Public Sub Copie()
  ...
  BDCopieSélection cDna, colChamps, bdSél
...
Private Sub Etat(ByVal iChoix As Integer, ByVal bImpression As Boolean)
  ...
  sSQLTête = "select unique dna_numgrp,grp_nom from dirnat,groupe where " & sFinalWP & " and dna_numgrp=grp_numgrp"
  sSQLCorps = "select * from dirnat where " & sFinalWP
  sSQL = "SHAPE {" & sSQLTête & "} AS Tête APPEND ({" & sSQLCorps & "} AS Corps RELATE 'dna_numgrp' TO 'dna_numgrp') AS Corps"

  Select Case iChoix
    Case 1: On Error Resume Next: BDEtat sSQL, drDirNatSimple, bImpression
    Case 2: On Error Resume Next: BDEtat sSQL, drDirNatDétaillé, bImpression
