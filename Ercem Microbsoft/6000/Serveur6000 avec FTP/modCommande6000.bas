...
Public Sub CommandeTCP6000(Index As Integer, ByVal sLigne As String)
  ...
  Select Case UCase(sCmd)
    ...
    Case "GETFTP":    CmdGetFTP Index, sArg
    Case "PUTFTP":    CmdPutFTP Index, sArg
...
Private Sub CmdHelp(Index As Integer)
  ...
  Retour Index, "GETFTP sys fdist floc Récupère fdist sur sys via FTP, le copie dans floc"
  Retour Index, "PUTFTP sys floc fdist Transfère floc sous le nom fdist sur sys via FTP"
...
Private Sub CmdGetFTP(Index As Integer, ByVal sArg As String)
  Dim sSystème As String
  Dim sFichierDistant As String
  Dim sFichierLocal As String
  
  sSystème = sGetArg(sArg)
  sFichierDistant = sGetArg(sArg)
  sFichierLocal = sGetArg(sArg)
  FinArguments sArg
  
  ' On récupère les données distantes
  Dim sURL As String
  sURL = "ftp://" & sSystème
  If Left(sFichierDistant, 1) <> "/" Then sURL = sURL & "/"
  sURL = sURL & sFichierDistant
  Dim sDonnées As String
  Dim b() As Byte
  Dim v
  On Error GoTo ProblèmeOpenURL
  v = frmServeur6000.FTP.OpenURL(sURL, icByteArray)
  On Error GoTo 0
  If VarType(v) = vbEmpty Then Err.Raise 6011, "CmdGetFTP", "Rien récupéré"
  b() = v

  Dim iFic As Integer
  iFic = FreeFile
  On Error GoTo ProblèmeOuverture
  Open sFichierLocal For Binary Access Write As #iFic
  On Error GoTo 0
  Put #iFic, , b()
  Close #iFic
  Exit Sub
...
ProblèmeOuverture:
  Err.Raise 6005, "CmdGetFTP", "Echec à l'ouverture de '" & sFichierLocal & "', Err=" & Err & ": " & Error
  
ProblèmeOpenURL:
  Err.Raise 6011, "CmdGetFTP", "Echec au FTP.OpenURL, Err=" & Err & ": " & Error
End Sub


Private Sub CmdPutFTP(Index As Integer, ByVal sArg As String)
  Dim sSystème As String
  Dim sFichierDistant As String
  Dim sFichierLocal As String
  ...
  Dim sURL As String, sCmd As String
  sURL = "ftp://" & sSystème
  sCmd = "PUT " & sFichierLocal & " " & sFichierDistant
  On Error GoTo ProblèmeExecute
  frmServeur6000.FTP.Execute sURL, sCmd
  On Error GoTo 0
  While frmServeur6000.FTP.StillExecuting
    DoEvents
  Wend
  Exit Sub

ProblèmeOuverture:
  Err.Raise 6005, "CmdPutFTP", "Echec à l'ouverture de '" & sFichierLocal & "', Err=" & Err & ": " & Error
  
ProblèmeExecute:
  Err.Raise 6011, "CmdPutFTP", "Echec au FTP.Execute '" & sURL & "', '" & sCmd & "' , Err=" & Err & ": " & Error
End Sub
...
Private Sub CmdMessage(Index As Integer, ByVal sArg As String)
  ...
  Else
    ...
    fMsg.lblOrigine = frmServeur6000.InfoConnexion(Index)
    fMsg.lblMessage = sMessage
    fMsg.Show
  End If
End Sub
...
Private Sub CmdMaj6000(Index As Integer, ByVal sArg As String)
  ...
  If Not frmServeur6000.bVersionPlusRécenteSurZ Then
    Retour Index, "!Pas de version plus récente sur Z:\microbsoft, ou disque Z: inaccessible"
  Else
