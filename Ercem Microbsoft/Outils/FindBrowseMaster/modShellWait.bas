...
Public Sub ShellWait(ByVal sCommande As String)
  ...
  hprog = Shell(sCommande, vbMinimizedNoFocus) 'returns taskID
  If Err Then
    MsgBox "Échec à la compilation." & vbCrLf & sCommande & vbCrLf & vbCrLf & "Err=" & Err & " (" & Err.Source & "): " & Err.Description, vbExclamation, sNomApp
    Return
  End If
  On Error GoTo 0
  'Get process handle
