...
Begin VB.Form frmRechercheUnité 
   Caption         =   "Recherche d'unité"
   ClientHeight    =   3975
   ClientLeft      =   1560
   ClientTop       =   2415
   ClientWidth     =   7020
   Icon            =   "frmRechercheUnité.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.TextBox txtRecherche 
      ...
      Width           =   3975
   End
   Begin MSComctlLib.ListView lvUnités 
      Height          =   2775
      Left            =   60
      TabIndex        =   2
      Top             =   1200
   ...
   End
End
Attribute VB_Name = "frmRechercheUnité"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Attribute VB_Description = "Feuille de recherche d'une unité à partir d'une partie de son nom"
' Feuille frmRechercheUnité
' Feuille de recherche d'une unité à partir d'une partie de son nom
' PV Octobre 96
...
Private Sub btnRechercher_Click()
  ...
  lvUnités.ListItems.Clear
  lvUnités.SetFocus
  
  Set Trouvé = RechercheUnité(txtRecherche)
  ...
  EffaceMarqueDeTri lvUnités
  lvUnités.Sorted = False
  Dim u
  For Each u In Trouvé
    Set itmX = lvUnités.ListItems.Add()
    itmX.Text = u.uni_numunite
    itmX.SubItems(1) = u.uni_code
    itmX.SubItems(2) = u.uni_titre
  Next
  LVSetAllColWidths lvUnités, LVSCW_AUTOSIZE_USEHEADER
  lblStatus = Trouvé.count & " unité(s) dont le nom contient " & txtRecherche
...
Private Sub Form_Resize()
  If ScaleWidth > 350 And ScaleHeight > 100 Then
    lvUnités.Width = ScaleWidth - 5
    lvUnités.Height = ScaleHeight - 82
...
Private Sub lvUnités_ColumnClick(ByVal ColumnHeader As ColumnHeader)
  TrieListView lvUnités, ColumnHeader.Index
End Sub

Private Sub lvUnités_DblClick()
  If btnOK.Enabled Then btnOk_Click
End Sub

Private Sub lvUnités_ItemClick(ByVal Item As ListItem)
  btnOK.Enabled = True
  btnOK.Default = True
...
Public Sub Internationalisation()
  ...
  SetLib lvUnités.ColumnHeaders(1), "Text", "RUchNo"
  SetLib lvUnités.ColumnHeaders(2), "Text", "RUchCode"
  SetLib lvUnités.ColumnHeaders(3), "Text", "RUchTitre"
End Sub
