...
static void near en_haut(int);
static void near en_bas(int);
static void near affiche_colonne(void);
static void near highlight(int);
static char * near joliformat(char *);
static void near selectparlettre(int);
  ...
  { // char buffer[200];
    ...
    switch (mode)
    {
      case 1: if (!exist(nomfic2))
		...
		i = iAlert("Attention !",buffer,1,2,"Changer de nom|Abandonner");
		if (i==2)
		  cr = 0;
		else
		  goto restart;
      ...
      case 3: if (exist(nomfic2))
	      { sprintf(buffer,"|Le fichier \"%s\" existe d‚j… !|",nomfic2);
		i = iAlert("Attention !",buffer,1,3,"Changer de nom|€a ne fait rien !|Abandonner");
		switch(i)
		{ case 3: cr=0; break;
		  case 1: goto restart;
		}
