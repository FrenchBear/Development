...
** 22/08/94 PV	Version extended, avec h:m:s
*/

long unixtimeex(int j, int m, int a, int hr, int min, int sec)
{
  int i;
  long dj;
  ...
  */
  ...
  tjm[1]=(a%4)?28:29;
  dj = 365L*(a-1970)+(a-1970+1)/4+j-1;
  for (i=0 ; i<m-1 ; i++)
    dj+=tjm[i];
  return ((dj*24+hr)*60+min)*60+sec;
}

long unixtime(int j, int m, int a)
{
  return unixtimeex(j,m,a,0,0,1); /* 1 sec aprŠs minuit */
}
