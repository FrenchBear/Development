...
const UString &usWidePath(const UString &s)
{
  wchar_t wsBuffer[32000];
  static UString us;

  if (!bWidePath)
	return s;

  if (wcslen((const wchar_t *)s)>=31990)
  {
	cout << "astruct: debordement du buffer de usWidePath, arret du programme\n";
	exit(1);
  }
  ...
  {
    wcscpy(wsBuffer, L"\\\\?\\");
	wcscat(wsBuffer, (const wchar_t *)s);
	us = UString(wsBuffer);
	return us;
  }
  else if (s[0]==L'\\' && s[1]==L'\\')
  {
    wcscpy((wchar_t *)wsBuffer, L"\\\\?\\UNC");
	wcscat((wchar_t *)wsBuffer, 1+(const wchar_t *)s);
	us = UString(wsBuffer);
	return us;
  }
  else
...
void Identification()
{
  cout << "ASTRUCT " << szVersion << (bModeW ? " (unicode long)" : " (ansi)") << endl
       << "Alignement d'une structure de dossiers sur disque\n\n";
}

inline void Usage()
{
  cout << "Usage: ASTRUCT [/?] [/t] [/v] [/%] [/_] [/l] [/p] [/s] [/w] source destination\n"
...
void CopieFichier(const UString &usNomficSource, const UString &usNomficDest)
{
  ...
  if (bModeW)
  {
	UString usSource = usWidePath(usNomficSource);
	UString usDest = usWidePath(usNomficDestLower);
	//cout << "$1: " << OEM(usSource) << endl;
	//cout << "$2: " << OEM(usDest) << endl;
	bRes = CopyFileW(usSource, usDest, FALSE);
  }
  else
...
void EffaceDir(const UString &usNomdir)
{
  ...
  usModele = usNomdir + L"\\*";
  if (bModeW)
	hFile = FindFirstFileW((const wchar_t *)usWidePath(usModele), &wfdw);
  else
  {
...
static void twp(UString &s)
{
	cout << OEM(s) << "  -->  " << OEM(usWidePath(s)) << endl;
}
...
int main(int argc, char **argv)
{
  ...
  cerr << iNbDirTotal << " dossier" << s(iNbDirTotal) << Aff(" analysé") << s(iNbDirTotal) << ", " << iNbFicTotal << " fichier" << s(iNbFicTotal) << Aff(" analysé") << s(iNbFicTotal) << endl
       << iNbCopy << " fichier" << s(iNbCopy) << Aff(" copié") << s(iNbCopy) << ", " << iNbDel << " fichier" << s(iNbDel) << Aff(" effacé") << s(iNbDel) << endl;
  if (iNbNoCopy)
