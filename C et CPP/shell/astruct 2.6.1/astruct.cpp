...
// 29/12/04 PV	2.5.0 Option /x: Exclut certains répertoires.
//					  Affiche des messages en anglais si la langue de Windows n'est pas le Français.
//  5/11/05 PV	2.5.2 Bug: Option /x n'était pas case insensitive
//  8/17/06 PV	2.5.3 Copie les dossiers commençant par un point
// 26/09/06 PV  2.6.0 Ne copie pas les hardlinks NTFS par défaut
// 28/09/06 PV  2.6.1 N'efface pas les répertoires SYSTEM sur le cible
...
bool bFollowhardLinks = false;	// Option h (copie les liens symboliques)
...
UString *tusExclusions = NULL;	// Tableau des répertoires à exclure
int iNbExclusions = 0;			// Nombre d'éléments du tableau qui précède
...
void Pause(bool bFinal)
{
	...
	if (bFinal && iNbNoCopy) cerr << Aff(bFra ? ", ?+Entrée pour revoir les problèmes" : ", ?+Enter to review problems");
	cerr << "...";
	cin.getline(c, sizeof(c));
	
	if (bFinal && c[0]=='?')
	{
		cerr << Aff(bFra ? "\nProblèmes rencontrés lors de l'exécution :\n" : "\nProblems encountered during program run :\n");
		Probleme *p;
...
void Identification()
{
	cout << "ASTRUCT " << szVersion << (bModeW ? (bWidePath ?" (unicode long)":" (unicode)") : " (ansi)") << endl
		 << (bFra ? "Alignement d'une structure de dossiers sur disque\n\n"
		          : "Align a folder tree on a disk\n\n");
}
...
inline void Usage()
{
	...
	cout << Aff("Source et destination sont deux dossiers existants\n"
		...
		"         /h      Copie le contenu des points de jonction (suit les 'hard links')\n"
		...
		"         /x dir  Exclut dir de la copie (ex: /x \"Temporary Internet Files\")\n") ;
	else
	cout << Aff("Source and destination must be valid and existing folders\n"
		"Options: /t      Deactivate time-difference control\n"
		"         /v      Verbose mode, detailed output (in French !)\n"
		"         /%      Do not copy files and folders beginning with %\n"
		"         /_      Do not copy files and folders beginning with  _\n"
		"         /l      Create files and folders with a lower case name\n"
		"         /h      Copy junctions contents (follow hard links)\n"
		"         /p      Final pause in case of problems, to review problems\n"
		"         /a      ANSI system calls forced under Windows NT/2K/XP\n"
		"         /i      ANSI output (default: OEM)\n"
		"         /w      WidePath, allows paths wider than 255 chars in Unicode mode\n"
		"         /n      NoAction, display commands but do not execute them\n"
		"         /s      Security, do not overwrite a more recent file on destination\n"
		"         /x dir  Exclude dir from copy (ie: /x \"Temporary Internet Files\")\n") ;
}
...
inline void Info()
{
	cout << (bFra ? "Fichier:          " : "File:             ") << __FILE__  << '\n'
		 ...
		 <<         "Copyright:        " << "(c) 1996-2006 P.VIOLENT";
}
...
void AStruct(UString usPathS, UString usPathD)
{
	...
	for(;;)
	{
		...
		{
			BOOL bExclu = false;
			for (int i=0 ; i<iNbExclusions ; i++)
				if (wcsicmp(wfdw.cFileName, tusExclusions[i])==0)
				{	bExclu = true;
					if (bVerbose)
						cout << Guillemets(Aff(wfdw.cFileName)) << " matching an exclusion, ignored.\n";
					break;
				}
			if ((!bExclu) && (!bFollowhardLinks) && (wfdw.dwFileAttributes&FILE_ATTRIBUTE_REPARSE_POINT)==FILE_ATTRIBUTE_REPARSE_POINT)
			{
				if (bVerbose)
					cout << Guillemets(Aff(wfdw.cFileName)) << " is a junction point, ignored.\n";
				bExclu = true;
			}
			if (!bExclu)
			{
	...
	for (i=0 ; i<iNbDest ; i++)
	{
		...
		*/
		...
		if (pSource==NULL ||	// N'existe plus sur la source
			((pDest->dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY) && (pSource->dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)==0) ||  
			...
			)
		{
			...
			{
				if ((pDest->dwFileAttributes&FILE_ATTRIBUTE_HIDDEN)==0 || (pDest->dwFileAttributes&FILE_ATTRIBUTE_SYSTEM)==0)	// On ignore les dossiers HIDDEN+SYSTEM
					EffaceDir(usAEffacer);
			}
			else							// Fichier
				EffaceFichier(usAEffacer);
...
void DetermineTimeOffset()
{
	...
	if (h<0)
	{
		cerr << Aff(bFra?"astruct (tGetTime): Échec à la création de ":"astruct (tGetTime): Failure during creation of ") << Guillemets(Aff(usPathSource)) << (bFra?": Erreur ":": Error ") << errno << ": " << sys_errlist[errno] << endl;
		Pause(0);
		exit(0);
	}
	close(h);
	...
	if (h<0)
	{ cerr << Aff(bFra?"astruct (tGetTime): Échec au stat ":"astruct (tGetTime): stat failed") << Guillemets(Aff(usPathSource)) << (bFra?": Erreur ":": Error ") << errno << ": " << sys_errlist[errno] << endl;
    if (bModeW)
	...
	if (h<0)
	{ 
		cerr << Aff(bFra?"astruct (tGetTime): Échec au stat ":"astruct (tGetTime): stat failed ") << Guillemets(Aff(usPathDest)) << (bFra?": Erreur ":": Error ") << errno << ": " << sys_errlist[errno] << endl;
	...
	if (abs(tDest-tSource)>2)
	{
		...
		cout << Aff("Attention, time difference between source and dest after copy is not supported\n")
			<< Aff("astruct does not manage this situation.\n")
			<< Aff("Option -t deactivates this test.\n");
...
int main(int argc, char **argv)
{
	...
	for (i=1 ; i<argc ; i++)
	{
		if (stricmp(argv[i], "/v")==0 || stricmp(argv[i], "-v")==0)
			...
			bTraceObjects = true;
		else if (stricmp(argv[i], "/h")==0 || stricmp(argv[i], "-h")==0)
			bFollowhardLinks = true;
		else if (stricmp(argv[i], "/x")==0 || stricmp(argv[i], "-x")==0)
		{
			iNbExclusions++;
			tusExclusions = (UString *)(realloc(tusExclusions, iNbExclusions*sizeof(UString)));
			memset(&tusExclusions[iNbExclusions-1], 0, sizeof(UString));
			char *p = argv[++i];
			if (*p=='"')
			{
				p++;
			...
			tusExclusions[iNbExclusions-1] = UnicodeFromANSI(p);
		}
		...
		else if (argv[i][0]=='/' || argv[i][0]=='-')
		{
			if (bFra)
				...
				cerr << "astruct: Unknown option " << argv[i] << endl;
			bProbleme = true;
		}
	}
	...
	for (i=1 ; i<argc ; i++)
	{
		...
		{
			/* Option déjà traitée */;
			// L'option -x prend un argument
			if (stricmp(argv[i], "/x")==0 || stricmp(argv[i], "-x")==0)
				i++;
	...
	if (iRangArg!=2)
	{
		cerr << Aff(bFra ? "astruct: La source et la destination doivent être précisées\n"
						 : "astruct: Source and destination must be specified\n");
		bProbleme = true;
	}
	else
	{
		if (!bOkDir(usSource))
		{
			cerr << "astruct: \"" << Aff(usSource) << (bFra ? "\" n'existe pas ou est inaccessible\n" : "\" does not exist, or is not accessible\n");
			bProbleme = true;
		}
		
		if (!bOkDir(usDest))
		{
			cerr << "astruct: \"" << Aff(usDest) << (bFra ? "\" n'existe pas ou est inaccessible\n" : "\" does not exist, or is not accessible\n");
			bProbleme = true;
		}
	}
	...
	cerr << iNbDirTotal << (bFra?" dossier":" folder") << s(iNbDirTotal) << Aff(bFra?" analysé":" analyzed") << (bFra?s(iNbDirTotal):"") << ", " << iNbFicTotal << (bFra?" fichier":" file") << s(iNbFicTotal) << Aff(bFra?" analysé":" analyzed") << (bFra?s(iNbFicTotal):"") << endl
		<< iNbCopy << (bFra?" fichier":" file") << s(iNbCopy) << Aff(bFra?" copié":" copied") << (bFra?s(iNbCopy):"") << ", " << iNbDel << (bFra?" fichier":" file") << s(iNbDel) << Aff(bFra?" effacé":" deleted") << (bFra?s(iNbDel):"") << (bNoAction ? " (noaction)":"") << endl;
	if (iNbNoCopy)
		cerr << iNbNoCopy << Aff(bFra?" problème":" problem") << s(iNbNoCopy) << (bFra?" lors de la copie":" during copy") << endl;
